...
1. Where in the world is the peninsula mapped above?
   It is the Iberian peninsula. 
   
2. Is altitude a climate variable? How do the peninsula maps plotted above support your answer?
   No, altitude is not a climate variable. The peninsula maps deal with extremes of climate only. The all.worldalt is the one that plots altitude.
...
Petrocoptis grandiflora and Petrocoptis pyrenaica are the accepted names for the species. 
...
They are both in family Caryophyllaceae.
...
```{r}
subspecies <- tax_name(q="Petrocoptis pyrenaica", get="subspecies", db="ncbi")
subspecies
```
...
P_grandiflora <- occ(q="Petrocoptis grandiflora", from="gbif", limit=500) # getting occurrence data
P_grandiflora_data <- (P_grandiflora$gbif$data)
P_grandiflora_df <- occ2df(P_grandiflora) # converting into dataframe
P_grandiflora_df <- P_grandiflora_df[complete.cases(P_grandiflora_df),] # cleaning dataframe
P_grandiflora_df
...
points(cbind(P_grandiflora_df$longitude, P_grandiflora_df$latitude), col="red", pch=20, cex=.8)
```
...
Petrocoptis grandiflora does occur on the Iberian peninsula. 
I do not think that Petrocoptis grandiflora is endemic to the Iberian peninsula because most of the occurence data occurs more North and West in Europe. 
...
This species occurs north of the Baja peninsula of California/Mexico.
...
I really am struggling with exercise 3. I will work on this in the portfolio edits. 
